...
\abstract{
    ...
    The models are discrete and work with species adaptation degrees (fitness) for natural selection.
    Implementations of simulation with Python language are performed, and fitness distributions graphics are obtained.
...
\resumo {
    ...
    Os modelos são discretos e trabalham com graus de adaptação das espécies (\textit{fitness}) para a seleção natural.
    Implementações de simulações utilizando a linguagem \textit{Python} são realizadas, e gráficos das distribuições dos \textit{fitness} são obtidos.
